package eval.question;

import java.util.Scanner;

public class Quest4_ControleSaisieNom {

	/**
	 * @param args
	 */
	public static void main(String[] args) {
		System.out.println("*******************************************************************\n" + "Question 4\n"
				+ "On demande la saisie d’un nom de famille.\n" + "La saisie sera redemandée tant que :\n"
				+ "	le nom contiendra un caractère numérique ,\n" + "	La longueur sera inférieure à 2,\n"
				+ "	La longueur sera supérieure à 10 .\n"
				+ "*******************************************************************\n\n\n");

		Scanner clavier = new Scanner(System.in);
		String chaine = null;
		boolean jeContinueADemander = true;

		while (jeContinueADemander) {

			jeContinueADemander = false;
			System.out.println("\nSaisissez un nom de famille : ");
			chaine = clavier.nextLine();

			if (chaine.length() < 2 || chaine.length() > 10) {
				jeContinueADemander = true;

			} else {
				for (int i = 0; i < chaine.length(); i++) {
					// :Character.isDigit(chaine.charAt(i));

					if ((int) chaine.charAt(i) >= 48 && (int) chaine.charAt(i) <= 57) {

						jeContinueADemander = true;
					}
				}

			}

		}
		
		System.out.println("nom de famille valide : " + chaine + " -Fin du programme ");

		clavier.close();
	}
}