package eval.question;

import java.util.Scanner;

public class Quest3_Addition {

	/**
	 * @param args
	 */
	public static void main(String[] args) {
		System.out.println(
"*******************************************************************\n" +
"Question 3\n" +
"L'utilisateur saisit 5 nombres maximum.\n"+
"Chaque nombre saisi est suivi de la touche [Entrée].\n"+
"La saisie est terminée quand les 5 nombres sont saisis ou si on entre le nombre zéro.\n"+
"Réafficher la liste des nombres saisis sous la forme d'une addition avec la somme de ses nombres.\n"+ 
"( exemple: 8 + 9 + 2 + 1  = 20 )\n"+
"*******************************************************************\n\n\n"
		);
		// programme qui affiche 5 nombres saisis
		
			Scanner clavier= new Scanner(System.in);
		    int [] nombres  =new  int [5];
		    int temp;
			
			
		    System.out.println("Entrez 5 nombres  ");
	       
		
			 for (int i=0; i<5; i++) {
				 
				 System.out.println("Saisissez le nombre no : " + (i+1)+ " : "
				 		+ "");	
				 temp=clavier.nextInt();
				 if(temp !=0) {
					 
					 nombres[i]=temp;
				 }else {
					 System.out.println("Saisie terminée par l'utilisateur ");
					 break;
				 }
			         
			 }
		/***********************************************/	 
			 String affichagefinal="";
			 int somme=5;
			 for (int i=0; i<5; i++) {
				 
				 somme+=nombres[i];
				 affichagefinal+=nombres[i];
				 
				 System.out.println(affichagefinal);
				 if(i !=4) {
					
					 if(nombres[i+1]==0)
						 break;
					 else
						 affichagefinal +="+";
				 }
			 }
			 
			 System.out.println(affichagefinal + " = " + somme ); 
			 
			 clavier.close();

}
	
	
}
